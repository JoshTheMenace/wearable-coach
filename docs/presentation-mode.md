...
Speech repetitions such as “Is Is this right?” and “Is this Is this right?” are normalized only for intent matching; the transcript is retained unchanged. “I think I'm I'm finished with that” also opens the recap. Negated requests still do not advance. Rejected navigation calls return silently to Gemini instead of starting another response/tool attempt while waiting for new input.
...
Reference search recognizes rescuer swapping, switching, changes, handoffs and handovers as relief queries. Previously “swapping rescuers during CPR” missed `relief` because “swapping” did not match “swap”; the incidental word “during” ranked an AED fact first. Search now returns the dataset's existing relief guidance first. Follow-up questions after the recap keep the tutor available without an unsolicited offer to end coaching.

Validation: 364 automated tests and the production build passed. A Gemini Live replay using the exact disfluent transcript advanced into practice, accepted completion, retrieved `relief` first for the handoff question and spoke a sourced answer while keeping coaching connected, with no rejected tools in that sequence. A separate synthesized-audio placement recheck also advanced after one clip. Playback reports were simulated; these checks did not use the physical glasses microphone.

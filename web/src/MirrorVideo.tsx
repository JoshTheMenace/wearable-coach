...
export function MirrorVideo({ demo, clip, now, mediaError, retryMedia }: {
  ...
  return <div className="mirror-video">
    {clip && <video ref={video} src={clip.localUrl} muted={muted} playsInline preload="auto" aria-label="Mirrored lesson video" />}
    ...
    {clip && demo.status === 'playing' && <button className="mirror-video-sound secondary" onClick={() => setMuted(value => !value)}>{muted ? 'Enable video sound' : 'Mute video sound'}</button>}
  </div>;
}

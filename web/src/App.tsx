...
export function App() {
  ...
  const media = useLessonMedia(session?.sessionId, session?.token, lessonMode, lessonMode && !!mockDevice && !session?.readOnly);
  ...
  useEffect(() => {
    ...
    if (isMock) {
      ...
      audioSocket.onmessage = message => {
        if (!(message.data instanceof ArrayBuffer) || message.data.byteLength < 24 || !soundRef.current || suppressed.current || snapshotRef.current?.demonstration || !audio.current) return;
  ...
  return <main className={lessonMode || (!session && cprMode) ? 'lesson-mode' : undefined}>
    ...
    {!session ? <section className="setup-grid">
      ...
      <div className="toolbar"><div className="session-info"><span className="badge">{snapshot?.config.provider ?? 'coach'}</span><span>{snapshot?.config.model}</span><span className="subtle">{snapshot?.config.device === 'mock' ? 'Glasses simulator · browser renderer' : snapshot?.config.device}</span></div><div className="toolbar-actions">{!session.readOnly && <><button onClick={toggleSound} className={lessonMode && !sound ? 'primary compact' : 'plain'}>{sound ? 'Sound on' : 'Enable sound'}</button><button onClick={reconnect} disabled={busy || !active} className="secondary compact">Reconnect</button><button onClick={() => send('end_session')} disabled={!canControl} className="danger compact">End session</button></>}<button onClick={leave} className="plain">Leave view ↗</button></div></div>
      ...
      />}
      {lessonMode && <div className="lesson-conversation">{conversation}</div>}
      <details className="lesson-lab-details" open={lessonMode ? undefined : true}><summary hidden={!lessonMode}>References, activity, and device details</summary><div>

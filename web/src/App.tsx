...
import { CprLesson } from './CprLesson.tsx';
...
export function App() {
  ...
  const media = useLessonMedia(session?.sessionId, session?.token, tutorMode, tutorMode && (snapshot?.config.device === 'meta_display' || !!mockDevice && !session?.readOnly));
  ...
  return <main className={tutorMode || (!session && cprMode) ? 'lesson-mode' : undefined}>
    ...
    {!session ? <section className="setup-grid">
      ...
      />}
      {tutorMode && snapshot?.config.device==='meta_display' && <LiveCameraPreview snapshot={snapshot as unknown as SessionSnapshot} token={session.token} now={serverNow} clips={media.clips} mediaLoading={media.loading} mediaError={media.error} retryMedia={media.retry} />}

...
function LiveVideoStatus({ snapshot, now }: { snapshot: Snapshot; now: number }) {
  ...
  return <section className="inspection-status" aria-label="Live camera status">
    ...
    <p>{age === null ? 'No frame received yet.' : stats?.cameraSource === 'recorded_video' ? `Recording frame received ${age}s ago · clip position ${((stats.sourcePositionMs ?? 0) / 1000).toFixed(1)}s. This is not the current learner.` : `Last reported frame received ${age}s ago; sensor capture age unknown.`}</p>
    <p>{snapshot.lesson ? 'Camera frames are sampled at ≤1 fps. During placement and practice, the observer checks hand position and sends its findings to the coach.' : 'Gemini reads sampled video directly (≤1 fps), without a separate observer check.'} Streamed frames are not recorded. {snapshot.config.device === 'mock' ? 'Use the glasses simulator to select a recorded camera feed.' : 'Start or stop it on Android.'}</p>
  </section>;
}
...
export function App() {
  ...
  const audioWrittenSamples = useRef(0);
  ...
  const simulatorCapture = useRef<(() => Promise<{ blob: Blob; meta: Record<string, unknown> }>) | null>(null);
  ...
  const tutorMode = lessonMode || snapshot?.config.tutorMode === 'marine';
  const media = useLessonMedia(session?.sessionId, session?.token, tutorMode, tutorMode && !!mockDevice && !session?.readOnly);
  ...
  const flushAudio = useCallback(() => {
    ...
    audioUntil.current = 0; audioWrittenSamples.current = 0;
  }, []);
  const reportDevice = useCallback((type: string, payload: Record<string, unknown>) => {
    const current = snapshotRef.current, socket = control.current;
    if (!current || ending.current || terminal(current.status) || socket?.readyState !== WebSocket.OPEN) return;
    socket.send(JSON.stringify({ schemaVersion: 1, sessionId: current.id, generation: current.generation, messageId: uuid(), type, payload }));
  }, []);
  const captureReady = useCallback((capture: (() => Promise<{ blob: Blob; meta: Record<string, unknown> }>) | null) => { simulatorCapture.current = capture; }, []);
  const uploadFrame = useCallback(async (blob: Blob, meta: Record<string, unknown>) => {
    if (!session) throw new Error('Session is unavailable');
    return await (await request(`/sessions/${session.sessionId}/frames/${uuid()}`, session.token, {
      method: 'POST', headers: { 'Content-Type': blob.type, 'x-frame-meta': JSON.stringify(meta) }, body: blob, signal: AbortSignal.timeout(5000),
  ...
  useEffect(() => {
    ...
    const receipt = (revision: number) => {
      ...
      report('hud.receipt', { hudRevision: revision, rendererInstanceId: renderer.current, target: 'mock', status: 'phone_received' });
    };
    const capture = async (data: Json) => {
      if (!isMock) return;
      try {
        if (simulatorCapture.current) {
          const frame = await simulatorCapture.current();
          if (!cancelled) await request(`/sessions/${session.sessionId}/frames/${uuid()}`, session.token, {
            method: 'POST', headers: { 'Content-Type': frame.blob.type, 'x-frame-meta': JSON.stringify({ ...frame.meta, generation, workId: data.workId }) }, body: frame.blob, signal: AbortSignal.timeout(5000),
          });
          return;
        }
      ...
      } catch (error) { if (!cancelled) { setError(errorMessage(error)); report('capture.failed', { workId: data.workId }); } }
    };
    ...
    if (isMock) {
      ...
      audioSocket.onmessage = message => {
        if (!(message.data instanceof ArrayBuffer) || message.data.byteLength < 24 || !soundRef.current || suppressed.current || (snapshotRef.current?.demonstration && snapshotRef.current.demonstration.status !== 'cueing') || !audio.current) return;
        ...
        if (Math.max(audioUntil.current, context.currentTime) - context.currentTime + count / rate > 60) { flushAudio(); setError('Coach audio exceeded the playback buffer. Ask the coach to repeat a shorter explanation.'); return; }
        ...
        audioWrittenSamples.current += count; sources.current.add(source); source.onended = () => sources.current.delete(source); source.start(starts);
      };
      audioSocket.onclose = () => { if (!cancelled) { suppressed.current = true; flushAudio(); } };
    }
    const playbackTimer = setInterval(() => {
      if (bound && snapshotRef.current?.demonstration?.status === 'cueing' && soundRef.current && audio.current) report('playback.metric', { speechEpoch: audioEpoch.current, metrics: { pendingMs: Math.max(0, audioUntil.current - audio.current.currentTime) * 1000, writtenSamples: audioWrittenSamples.current }, measurementBasis: 'browser_scheduled_audio' });
    }, 250);
    ...
    return () => { cancelled = true; clearTimeout(recoveryTimer); clearInterval(pendingTimer); clearInterval(playbackTimer); setControlReady(false); socket.close(); audioSocket?.close(); flushAudio(); };
  ...
  return <main className={tutorMode || (!session && cprMode) ? 'lesson-mode' : undefined}>
    ...
    {!session ? <section className="setup-grid">
      ...
      <div className="toolbar"><div className="session-info"><span className="badge">{snapshot?.config.provider ?? 'coach'}</span><span>{snapshot?.config.model}</span><span className="subtle">{snapshot?.config.device === 'mock' ? 'Glasses simulator · browser renderer' : snapshot?.config.device}</span></div><div className="toolbar-actions">{!session.readOnly && <><button onClick={toggleSound} className={tutorMode && !sound ? 'primary compact' : 'plain'}>{sound ? 'Sound on' : 'Enable sound'}</button><button onClick={reconnect} disabled={busy || !active} className="secondary compact">Reconnect</button><button onClick={() => send('end_session')} disabled={!canControl} className="danger compact">End session</button></>}<button onClick={leave} className="plain">Leave view ↗</button></div></div>
      {snapshot && lessonMode && <CprLesson snapshot={snapshot as unknown as SessionSnapshot} intro={media.manifest?.intro} ready={canControl} readOnly={session.readOnly} now={serverNow} send={send} />}
      {media.error && <div className="alert" role="alert"><span>{media.error}</span><button className="secondary compact" onClick={media.retry}>Retry lesson media</button></div>}
      {snapshot && mockDevice && !session.readOnly && <GlassesSimulator

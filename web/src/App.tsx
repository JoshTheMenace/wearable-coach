...
function LiveVideoStatus({ snapshot, now }: { snapshot: Snapshot; now: number }) {
  ...
  return <section className="inspection-status" aria-label="Live camera status">
    ...
    <p>Gemini reads sampled video directly (≤1 fps), without a separate observer check. Streamed frames are not recorded. {snapshot.config.device === 'mock' ? 'Use the glasses simulator to select a recorded camera feed.' : 'Start or stop it on Android.'}</p>
  </section>;
}
